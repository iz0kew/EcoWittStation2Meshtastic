...
- **Bridge Meshtastic** (opzionale, configurabile):
  ...
  - NodeInfo con short/long name personalizzabili
...
send_interval_min = 10            # intervallo tra invii telemetria (minuti)
...
lightning_text = true             # avviso testo quando WH57 rileva fulmini
...
2. **Dopo ~1 minuto**: NodeInfo inviato → compare con short/long name configurati
3. **Contestualmente**: posizione fissa inviata → appare sulla mappa
4. **Ogni `send_interval_min`**: `EnvironmentMetrics` con media temperatura/umidità e pioggia 1h/24h
5. **Ogni ora**: NodeInfo e posizione aggiornati
6. **Se `lightning_text = true`**: messaggio sul canale tipo `Fulmini: 3 (ultimo ~12 km)`
...
- **Meshtastic bridge** (optional, fully configurable):
  ...
  - NodeInfo with configurable short/long name
...
preset = MediumFast               # LoRa modem preset
                                  ...
                                  # MediumFast | MediumSlow |
                                  # LongFast | LongModerate | LongSlow
send_interval_min = 10            # telemetry send interval (minutes)
...
lightning_text = true             # send text alert when WH57 detects lightning
...
2. **After ~1 minute**: NodeInfo sent → appears with the configured short/long name
3. **At the same time**: fixed position sent → appears on the map
4. **Every `send_interval_min`**: `EnvironmentMetrics` with average temperature/humidity and rainfall 1h/24h
5. **Every hour**: NodeInfo and position refreshed
6. **If `lightning_text = true`**: channel message such as `Fulmini: 3 (ultimo ~12 km)`
