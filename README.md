# EcoWittStation2Meshtastic — v1.4.0
...
- **Display multi-schermata** con navigazione tramite tasto PRG:
  ...
  | 9 | Orario Mesh (sincronizzato dalla rete) |
  | 10 | Effemeridi (alba, tramonto, fase lunare) |

- **Storico circolare** in RAM: campionatura ogni 5–10 min, finestra 24 ore

- **Orologio sincronizzato dalla rete Meshtastic** (senza GPS né NTP):
  - In ascolto LoRa nei primi 5 minuti per ricevere un timestamp dalla mesh
  - **Fuso orario automatico** derivato dalle coordinate di `settings.ini` (DST incluso)

- **Calcoli astronomici offline** basati su coordinate e ora locale:
  - Alba e tramonto (algoritmo NOAA, ±1 min per latitudini 0°–65°)
  - Fase lunare: nome completo con indicazione crescente/calante, percentuale di illuminazione
  - **Scheduler astronomico**: bollettino su canale principale (MediumFast) inviato automaticamente a alba+1h, mezzogiorno locale e tramonto−1h
  - Bollettino periodico su canale secondario (METEOLAZIO) con intervallo `text_interval_min` separato
...
└── src/
    ...
    ├── screens.cpp               le 10 schermate (incl. orologio ed effemeridi)
    ├── timesync.h / .cpp         sincronizzazione orario dalla rete Meshtastic
    ├── astro.h / .cpp            calcoli astronomici: alba/tramonto e fase lunare
...
- **Multi-screen display** with navigation via the PRG button:
  ...
  | 9 | Mesh clock (network-synchronised) |
  | 10 | Ephemeris (sunrise, sunset, moon phase) |

- **Circular history** in RAM: one sample every 5–10 min, 24-hour window

- **Clock synchronised from the Meshtastic network** (no GPS, no NTP):
  - Listens on LoRa for the first 5 minutes to receive a timestamp from the mesh
  - **Automatic timezone** derived from the coordinates in `settings.ini` (DST included)

- **Offline astronomical calculations** based on coordinates and local time:
  - Sunrise and sunset (NOAA algorithm, ±1 min for latitudes 0°–65°)
  - Moon phase: full name with waxing/waning indication, illumination percentage
  - **Astronomical scheduler**: bulletin sent on the primary channel (MediumFast) automatically at sunrise+1h, local noon, and sunset−1h
  - Periodic bulletin on the secondary channel (METEOLAZIO) at a separate `text_interval_min` interval
